...
public class PublishAllGridnodesIntegrityTest extends BaseCodecTest<PublishAllGridnodes>{
	@Mocked
	private Connection emptyConnection;
	
	@Provide
	public Arbitrary<PublishAllGridnodes> provideGridnodePacket(@ForAll @AlphaChars @StringLength(36) String gridnodeKey,
		@ForAll @IntRange(min = 4097, max = 65535) int port) throws UnknownHostException {

		final PublishAllGridnodes gp = PublishAllGridnodes.builder().build();
		final String ip = ArbitraryGenerator.ip4();
		final Gridnode gridnode = Gridnode.builder().id(gridnodeKey).hostName(ip + ":" + port).build();
		gp.getGridnodes().add(gridnode);
	...
	@Property
	@SneakyThrows
	public void shouldMatch(@ForAll("provideGridnodePacket") PublishAllGridnodes gridnodePacket, 
		@Mocked ChannelHandlerContext context) {

		final Optional<Pair<MutableInt, MutableInt>> sizes = getSizeHolder();

		final PublishAllGridnodes resultingGridnode = encodeDecode(gridnodePacket,
			new PublishAllGridnodesEncoder(), new PublishAllGridnodesDecoder(), context, sizes
		);
