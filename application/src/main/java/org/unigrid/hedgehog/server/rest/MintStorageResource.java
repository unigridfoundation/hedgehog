...
public class MintStorageResource extends CDIBridgeResource {
	...
	@Path("/mint-storage/{address}/{height}")
	@PUT
	public Response grow(@NotNull GrowMint growMint, @NotNull @PathParam("address") String address,
		...
		try {
			...
			if (signature.verifyMultiple(growMint.getData().getBytes(), growMint.getSignatures())
				...
				final MintStorage.SporkData sporkData = ms.getData();
				final BigDecimal oldMintAmount = sporkData.getMints().get(location);
